...
const DetailPage: React.FC<any> = ({ authService }) => {
  ...
  const data =
    productData &&
    (match.path === '/cart'
      ? productData[loginMail].cart
      : productData[loginMail].product);
  const messgae =
    match.path === '/cart' ? '내가 담은 목록' : '내가 등록한 상품';
  ...
  return (
    <DetailWrapper>
      ...
      {data ? (
        <Product userData={data} user={loginMail} />
      ) : (
        <div>담아둔 상품이 없습니다.</div>
      )}
    </DetailWrapper>
  );
};
